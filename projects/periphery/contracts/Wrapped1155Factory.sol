...
// File: @openzeppelin/contracts@3.4.0/token/ERC1155/IERC1155.sol
...
// File: @openzeppelin/contracts@3.4.0/token/ERC1155/IERC1155Receiver.sol
...
// File: @openzeppelin/contracts@3.4.0/token/ERC1155/ERC1155Receiver.sol
...
contract Wrapped1155Factory is ERC1155Receiver {
    ...
    function toFixedBytes(bytes memory data, uint256 length) internal pure returns (bytes memory) {
        bytes memory result = new bytes(length);
        for (uint256 i = 0; i < length && i < data.length; i++) {
            result[i] = data[i];
        }
        return result;
    }
    ...
    {
        bytes memory tokenName = new bytes(32);
        for (uint256 i = 0; i < 32; i++) {
            tokenName[i] = data[i];
        }

        bytes memory tokenSymbol = new bytes(32);
        for (uint256 i = 0; i < 32; i++) {
            tokenSymbol[i] = data[32 + i];
        }

        bytes memory tokenDecimal = new bytes(1);
        tokenDecimal[0] = data[64];
