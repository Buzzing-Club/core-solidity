...
/// @title Canonical BuzzingSwap V3 factory
/// @notice Deploys BuzzingSwap V3 pools and manages ownership and control over pool protocol fees
contract BuzzingSwapFactory is IBuzzingSwapFactory {
    /// @inheritdoc IBuzzingSwapFactory
    ...
    function createPool(
        ...
        pool = IBuzzingSwapPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IBuzzingSwapPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IBuzzingSwapPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IBuzzingSwapPool(pool).setLmPool(lmPool);
    }
}
