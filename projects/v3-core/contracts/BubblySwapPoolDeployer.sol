...
import './BubblySwapPool.sol';

contract BubblySwapPoolDeployer is IBubblySwapPoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc IBubblySwapPoolDeployer
    ...
    function deploy(
        ...
        pool = address(new BubblySwapPool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
