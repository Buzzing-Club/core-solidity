...
/// @title Canonical BubblySwap V3 factory
/// @notice Deploys BubblySwap V3 pools and manages ownership and control over pool protocol fees
contract BubblySwapFactory is IBubblySwapFactory {
    /// @inheritdoc IBubblySwapFactory
    ...
    function createPool(
        ...
        pool = IBubblySwapPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IBubblySwapPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IBubblySwapPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IBubblySwapPool(pool).setLmPool(lmPool);
    }
}
