...
/// versions.
contract BuzzingSwapLmPoolDeployer {
    struct Parameters {
        address pool;
        address masterChef;
    }
    ...
    function deploy(address pool) external onlyMasterChef returns (address lmPool) {
        ...
        lmPool = address(new BuzzingSwapLmPool{salt: keccak256(abi.encode(pool, masterChef, block.timestamp))}());

        delete parameters;

        // Set new LMPool for pancake v3 pool.
        IBuzzingSwapFactory(INonfungiblePositionManager(IMasterChefV3(masterChef).nonfungiblePositionManager()).factory())
