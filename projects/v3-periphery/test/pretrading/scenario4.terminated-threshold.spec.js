...
describe("PreTrading Scenario 4", function () {
  it("when total deposits exceed threshold, market becomes TERMINATED immediately", async function () {
    ...
    const { oracle, u1, u2, preTrading } = await deployPreTradingFixture(threshold);
    const cond = ethers.utils.formatBytes32String("TERM_TEST");
    await (await preTrading.connect(oracle).setMarketTransferThreshold(cond, threshold)).wait();
    ...
    await expect(preTrading.connect(u2).deposit(cond, false, usdcAmount(3))).to.be.revertedWith("Market not open");
  });
});
