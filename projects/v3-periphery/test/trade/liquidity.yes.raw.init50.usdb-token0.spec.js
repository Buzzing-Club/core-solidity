...
async function buildUsdbToken0Market(env) {
  const { ctx, core, business, poolArtifact } = env;

  for (let i = 801; i <= 860; i += 1) {
    ...
    return {
      ...
      yesPositionId,
      yesTokenAddr,
      token0: business.usdb.address,
      token1: yesTokenAddr,
    };
  }

  throw new Error("Could not build market with USDB=token0");
}

describe("YES raw add when USDB=token0, initTick=50", function () {
  it("tries pure YES raw in range [0, 50]", async function () {
    this.timeout(300000);

    const env = await setupBaseEnv();
    const market = await buildUsdbToken0Market(env);
    const [operator, recipient] = await ethers.getSigners();
    const rawYes = ethers.utils.parseUnits("1000", 6); // 1000000000

    await (await market.poolContract.initialize(tickToSqrtPriceX96(50))).wait();
    ...
    console.log("[yes-raw-init50] operator=", operator.address, "recipient=", recipient.address);
    console.log("[yes-raw-init50] pool=", market.pool, "token0=", market.token0, "token1=", market.token1);
    console.log("[yes-raw-init50] pool.token0()=", poolToken0, "pool.token1()=", poolToken1);
    console.log(
      "[yes-raw-init50] currentTick=",
      Number(slot0.tick),
      "tickLower=",
