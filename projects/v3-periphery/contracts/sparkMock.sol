...
contract SUsds {
    ...
    // Savings yield
    UsdsJoinLike public  usdsJoin;
    VatLike      public  vat;
    UsdsLike     public  usds;
    address      public  vow;
    ...
    constructor(address usdsJoin_ ) {
        chi = uint192(RAY);
        rho = uint64(block.timestamp);
        ssr = 1_000_000_000_000_000_000_000_100_000;
        //vat.hope(address(usdsJoin));
        wards[msg.sender] = 1;
        usdsJoin = UsdsJoinLike(usdsJoin_);
        usds = UsdsLike(usdsJoin_);
    }
    ...
    function drip() public returns (uint256 nChi) {
        ...
        if (block.timestamp > rho_) {
            ...
            usdsJoin.mint(address(this), diff);
